...
import { getInvitationById } from '@adaven/platform-core';
...
export default function InviteScreen() {
  ...
  const handleInvitation = async () => {
    try {
      const invitationId = params.id ?? params.token;
      ...
      if (authUser) {
        const { data: userData } = await getPlatformClient()
          .from('users')
          .select('email')
          .eq('id', authUser.id)
          .single();
      ...
      const { data: existingUser } = await getPlatformClient()
        .from('users')
        .select('email')
