...
export default function HomeScreen() {
  ...
  useEffect(() => {
    if (Platform.OS === 'web' || currentSpace?.kind !== 'provider' || !currentSpace?.id) {
      return;
    }
    let cancelled = false;
      ...
      showChoiceDialog(
        ...
        [
          { text: 'Cancel', onPress: () => {}, style: 'cancel' },
          {
            ...
            onPress: () => {
              void (async () => {
                if (currentSpace?.kind === 'consumer' && currentSpace.id) {
  ...
  const isFirmPending = currentSpace?.kind === 'provider' && currentSpace?.firmStatus !== 'approved';
    ...
    return (
      <>
        <View style={styles.container}>
          <StatusBar style="dark" />
          {currentSpace?.kind === 'provider' ? <CrmDashboardView /> : <WebDashboardView />}
        </View>
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      {currentSpace?.kind === 'provider' && !isFirmPending ? (
