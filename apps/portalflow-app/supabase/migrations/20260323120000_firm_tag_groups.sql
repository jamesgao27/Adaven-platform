...
BEGIN
  ...
  FROM firm.clients c
  JOIN public.spaces s ON s.id = c.client_space_id AND s.kind = 'consumer'
...
BEGIN
  IF NEW.is_admin IS TRUE AND (TG_OP = 'INSERT' OR COALESCE(OLD.is_admin, false) IS DISTINCT FROM true) THEN
    IF EXISTS (SELECT 1 FROM public.spaces s WHERE s.id = NEW.space_id AND s.kind = 'provider') THEN
      INSERT INTO firm.group_members (group_id, user_id)
