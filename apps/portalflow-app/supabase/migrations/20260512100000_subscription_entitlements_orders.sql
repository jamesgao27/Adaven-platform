...
BEGIN
  ...
  v_sku_code := CASE WHEN NEW.kind = 'provider' THEN 'FIRM_TRIAL_30' ELSE 'CLIENT_TRIAL_30' END;
...
INNER JOIN crm.sku_edition se ON se.code = CASE WHEN s.kind = 'provider' THEN 'FIRM_TRIAL_30' ELSE 'CLIENT_TRIAL_30' END
WHERE NOT EXISTS (
  SELECT 1
  FROM crm.space_orders so
