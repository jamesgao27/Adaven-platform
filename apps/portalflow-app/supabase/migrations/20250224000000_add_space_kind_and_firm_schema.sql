-- =============================================================================
-- 1. 为 spaces 增加 kind 标记：consumer | provider（UI：Client | Firm）
-- 2. 新建 schema firm，Firm 端功能数据表存于此 schema
-- =============================================================================

-- 1. spaces 增加 kind，默认 consumer（Client）
ALTER TABLE public.spaces
  ADD COLUMN IF NOT EXISTS kind TEXT NOT NULL DEFAULT 'consumer'
    CHECK (kind IN ('consumer', 'provider'));

COMMENT ON COLUMN public.spaces.kind IS 'Kernel: provider (Firm) or consumer (Client).';
