...
- **Enable Custom SMTP** 打开，填写：
  ...
  - **Sender name**：发件人名称（如 Portalflow）
...
邀请邮件里的“接受邀请”链接会跳转到你的应用（如 `exp://localhost:8081/invite/xxx` 或 `portalflow://invite/xxx`）。**必须**把这些地址加入白名单，否则 Supabase 可能不发邮件或链接点不开。

- Dashboard → **Authentication** → **URL Configuration**
- 在 **Redirect URLs** 中**新增**（每行一个）：
  - 开发：`exp://localhost:8081/**`
  - 生产（如用 deep link）：`portalflow://**`
...
1. **Redirect URLs 是否包含邀请跳转地址**  
   若缺少 `exp://localhost:8081/**` 或 `portalflow://**`，Supabase 可能拒绝发信或链接无效。补全后重试。
